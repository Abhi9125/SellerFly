  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://your-cloud-function-url/sendContactEmail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ name, email, number, requirements, language }),
        }
      );
      ...
      if (response.ok) {
        ...
        setFormStatus(
          "Error sending email notification, but form submitted successfully."
        );
      }
